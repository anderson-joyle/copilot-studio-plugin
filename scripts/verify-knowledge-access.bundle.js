...
function loadConn(agentDir) {
  ...
  if (!fs.existsSync(connPath)) {
    die(
      `No .mcs/conn.json at ${connPath}. Is this a Copilot Studio agent cloned with 'pac copilot clone'? You can instead pass --tenant-id and --client-id directly.`
    );
  }
...
var AUTHOR_NOTE = "This confirms the signed-in (author) user's access only. SharePoint/OneDrive knowledge is retrieved at runtime using each end user's permissions, so ensure end users of the agent also have access to this item.";
async function main() {
  ...
  if (!tenantId) {
    die(
      "No tenant id. Pass --tenant-id <guid>, or --agent-dir <path> to a cloned agent whose .mcs/conn.json carries the tenant id."
    );
  }
  if (!clientId) {
    die(
  ...
  if (res.status === 404) {
    emit({
      ...
      httpStatus: 404,
      checkedAs: "author",
      note: "The link could not be resolved (item not found). Check that the URL points at an existing file or folder \u2014 open it in the browser and copy the address-bar URL."
    });
    return;
  }
  if (res.status === 401) {
